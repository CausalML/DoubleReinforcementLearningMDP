...
def regress_mu(s, a, w_, squared=False):
    N, T = s.shape
    mu_weights = []
    for t in range(T):
        s_t = s[:, t]**2 if squared else s[:, t]
        sa = torch.stack([s_t, s_t * a[:, t], torch.ones(N, device=device)], dim=1)
        y = w_[:, t]
        mu_weights.append(linear_regression(sa, y))
    return mu_weights
...
def eval_dr2(q1, q2, mu1, mu2, s1, s2, a1, a2, r1, r2, alpha, squared_q=False, squared_mu=False):
    ...
    def compute_half(qw, mw, s, a, r):
        ...
        for t in range(s.shape[1]):
            ...
            # # This matches the original toytoy2.py implementation that only considers action 1
            # a_val = 1.0  # Only consider action 1
            # sa_val = torch.stack([s_t, s_t * a_val, torch.ones(len(s), device=device)], dim=1)
            # a_val_tensor = torch.full_like(s[:, t], a_val)
            # V_t = eval_dens(s[:, t], a_val_tensor, alpha) * (sa_val @ qw[t]).squeeze()
...
def run_single_repetition(N, T, beta, alpha):
    ...
    mu1 = regress_mu(s1, a1, w1)
    mu2 = regress_mu(s2, a2, w2)
    mu1_sq = regress_mu(s1, a1, w1, squared=True)
    mu2_sq = regress_mu(s2, a2, w2, squared=True)
...
def main():
    ...
    print(f"Running with parameters: N={args.N}, T={args.T}, beta={args.beta}, alpha={args.alpha}, reps={args.reps}")
    
    # For very first repetition, get keys from a small sample run
    # This prevents out-of-memory errors when N is large
    sample_N = min(100, args.N)
    sample_res = run_single_repetition(sample_N, args.T, args.beta, args.alpha)
    ...
    for i in range(args.reps):
        ...
        # Run repetition
        try:
            res = run_single_repetition(args.N, args.T, args.beta, args.alpha)
            # Collect results
            for k in results:
            ...
            if (i+1) % 10 == 0 or i == args.reps-1:
                for k in results:
                    np.savez(f"estimator_list_{k}_{args.N}", a=np.array(results[k]))
                print(f"Saved checkpoint after {i+1} repetitions")
                
        except Exception as e:
            print(f"Error in repetition {i+1}: {e}")
            # Try to continue with next repetition
