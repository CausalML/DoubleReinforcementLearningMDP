...
for iii in range(rep): 
    ...
    ### DR estiamtor (Cross fitting)
    
    ############### Make q-function 
    ################################
    r_list_1 = r_list[0:N/2,:]
    r_list_2 = r_list[N/2:N,:]
    s_list_1 = s_list[0:N/2,:]
    s_list_2 = s_list[N/2:N,:]
    a_list_1 = a_list[0:N/2,:]
    a_list_2 = a_list[N/2:N,:]
    w_list_1 = w_list[0:N/2,:]
    w_list_2 = w_list[N/2:N,:]
    ...
    for j in bbb[::-1]:
        if j==(T-1):
            X = np.array([s_list_1[:,j],a_list_1[:,j]])
            pre_X = np.array([s_list_1[:,j],a_list_1[:,j]])
            Y = r_list_1[:,j]
        else:
            X = np.array([s_list_1[:,j],a_list_1[:,j]])
            aaa = []
            for k in range(N/2):
                aaa.append(eval_policy_dens(s_list_1[k,j+1],1,0))
            X0 = np.array([s_list_1[:,j+1],aaa])
            Y = r_list_1[:,j]+reg.predict(np.transpose(X0))
        reg = linear_model.LinearRegression()
        reg.fit(np.transpose(X), Y)
        ###print reg.score(np.transpose(X), Y)
        reg_list.append(reg)
    ...
    dr = 0.0
    for t in range(T):
        dr = dr + np.mean(r_list_2[:,t]*w_list_2[:,t])
        #### q function
        X = np.array([s_list_2[:,t],a_list_2[:,t]])
        dr = dr - np.mean(reg_list[T-1-t].predict(np.transpose(X))*w_list_2[:,t])
        #### v function 
        aaa = []
        for i in range(N/2):
            aaa.append(eval_policy_dens(s_list_2[i,t],1,0))
        X0 = np.array([s_list_2[:,t],aaa])
        if t==0:
            dr = dr + np.mean(reg_list[T-1-t].predict(np.transpose(X0)))
        else:
            dr = dr + np.mean(reg_list[T-1-t].predict(np.transpose(X0))*w_list_2[:,t-1])
        
    reg_list_2 = []
    for j in bbb[::-1]:
        if j==(T-1):
            X = np.array([s_list_2[:,j],a_list_2[:,j]])
            pre_X = np.array([s_list_1[:,j],a_list_2[:,j]])
            Y = r_list_2[:,j]
        else:
            X = np.array([s_list_2[:,j],a_list_2[:,j]])
            aaa = []
            for k in range(N/2):
                aaa.append(eval_policy_dens(s_list_2[k,j+1],1,0))
            X0 = np.array([s_list_2[:,j+1],aaa])
            Y = r_list_2[:,j]+reg.predict(np.transpose(X0))
        reg = linear_model.LinearRegression()
        reg.fit(np.transpose(X), Y)
        ###print reg.score(np.transpose(X), Y)
        reg_list_2.append(reg)
        
    for t in range(T):    
        dr = dr + np.mean(r_list_1[:,t]*w_list_1[:,t])
        #### q function
        X = np.array([s_list_1[:,t],a_list_1[:,t]])
        dr = dr - np.mean(reg_list_2[T-1-t].predict(np.transpose(X))*w_list_1[:,t])
        #### v function 
        aaa = []
        for i in range(N/2):
            aaa.append(eval_policy_dens(s_list_2[i,t],1,0))
        X0 = np.array([s_list_1[:,t],aaa])
        if t==0:
            dr = dr + np.mean(reg_list_2[T-1-t].predict(np.transpose(X0)))
        else:
            dr = dr + np.mean(reg_list_2[T-1-t].predict(np.transpose(X0))*w_list_1[:,t-1])
    
    estimator_list_dr.append(dr/2.0)
    ####print dr/2.0
    ...
    ############### DR estiamtor under M_2 (cross fitting) ###############
     ###############
         ###############
    r_list_1 = r_list[0:N/2,:]
    r_list_2 = r_list[N/2:N,:]
    ...
    for t in range(T):
        X = np.array([s_list_2[:,t],a_list_2[:,t]])
        dr2 = dr2 + np.mean(wreg_list[T-1-t].predict(np.transpose(X))*r_list_2[:,t])
        #### q function
        ...
        if t==0:
            ...
            X_ = np.array([s_list_2[:,t-1],a_list_2[:,t-1]])
    ...
    for j in bbb[::-1]:
        ...
        reg.fit(np.transpose(X),Y)
        ###print reg.score(np.transpose(X), Y)
        wreg_list_2.append(reg)
        
    for t in range(T):
        X = np.array([s_list_1[:,t],a_list_1[:,t]])
        dr2 = dr2 + np.mean(wreg_list_2[T-1-t].predict(np.transpose(X))*r_list_1[:,t])
        #### q function
        dr2 = dr2 - np.mean(reg_list_2[T-1-t].predict(np.transpose(X))*wreg_list_2[T-1-t].predict(np.transpose(X)))
        #### v function 
        aaa = []
        for i in range(N/2):
            aaa.append(eval_policy_dens(s_list_1[i,t],1,0))
        X0 = np.array([s_list_1[:,t],aaa])
        if t==0:
            dr2 = dr2 + np.mean(reg_list_2[T-t-1].predict(np.transpose(X0))) 
        else:
            X_ = np.array([s_list_1[:,t-1],a_list_1[:,t-1]])
            dr2 = dr2 + np.mean(reg_list_2[T-1-t].predict(np.transpose(X0))*wreg_list_2[T-t].predict(np.transpose(X_)))
    estimator_list_dr2.append(dr2/2.0)
    #####print(dr2/2.0)
